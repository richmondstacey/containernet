    ...
    def __init__(self, name, dimage=None, dcmd=None, build_params={},
        ...
        * updateMemoryLimits(...)
        ...
        self.dc = self.dcli.create_container(
            ...
            volumes=[self._get_volume_mount_name(v) for v in self.volumes if
                     self._get_volume_mount_name(v) is not None],
            hostname=name
        )
...
class DockerSwarm(Docker):
    def __init__(self, name, dimage=None, dcmd=None, build_params={}, **kwargs):
        ...
        task = docker.types.TaskTemplate(
            docker.types.ContainerSpec(self.dimage, command=self.dcmd, hostname=name, tty=True, open_stdin=True,
                                       cap_add=self.cap_add))
        response = self.dcli.create_service(task, self.NameService,
                                            mode=docker.types.ServiceMode("replicated", self.numRep))
